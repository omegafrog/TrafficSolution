...
이 저장소는 공공데이터 API를 이용해 도로에 설치된 VDS 데이터를 받아오고, 고속도로 혼잡도와 CCTV 정보를 조회하는 .NET 10 기반 C# WinForms 애플리케이션이다.

# Build/Test Commands

- `dotnet build TrafficSolution.slnx`: 솔루션 빌드. 현재 Linux 환경에서는 `NETSDK1100`으로 실패한 이력이 있으므로, Windows 또는 Windows Desktop SDK/runtime가 있는 환경에서 다시 검증해야 한다.
- `dotnet test TrafficSolution.slnx`: 전체 단위 테스트 실행. 현재 Linux 환경에서는 `NETSDK1100`으로 실패한 이력이 있으므로, Windows 또는 Windows Desktop SDK/runtime가 있는 환경에서 다시 검증해야 한다.
- `python3 scripts/validate_docs.py`: `docs/` 아래 계획/설계 문서 구조와 링크를 검증한다.

If a command becomes unavailable or changes, update this section with the real command instead of guessing.

# Documentation Navigation

- [docs/design-docs](docs/design-docs/index.md): architecture decisions, design notes, and verification artifacts
- [docs/product-specs](docs/product-specs/index.md): user requirements, functional requirements, and use cases
- [docs/exec-plans/active](docs/exec-plans/active/index.md): active execution plans
- [docs/exec-plans/completed](docs/exec-plans/completed/index.md): completed execution plans
...
- `docs/` is the source of truth when code and docs disagree.
...
- `Form` 코드는 UI 계층으로만 취급하고, 비즈니스 로직은 Service/Domain 쪽으로 밀어넣는다.
- `Program.cs`가 Composition Root이며, 외부 의존성은 Port/Adapter 패턴으로 주입한다.
- 좌표계 최종 출력과 외부 인터페이스는 항상 `EPSG:4326`을 사용한다. 내부 연산에서 `EPSG:3857`을 써도 최종 반환 전에 변환한다.
- 조회 좌표는 남한 범위(`MIN_LATITUDE = 33.0`, `MAX_LATITUDE = 39.0`, `MIN_LONGITUDE = 125.0`, `MAX_LONGITUDE = 132.0`) 안에서만 사용하고 bounds 기반 필터링을 유지한다.
- 공공데이터 API 키는 `SERVICE_KEY` 환경변수로만 주입하고, 소스코드/문서/커밋에 실제 값을 남기지 않는다.
- 신규/수정 테스트 코드는 반드시 `TestProject1` 프로젝트에 작성한다.
- 모든 커스텀 예외는 `ExceptionCode`와 `Description`을 포함해야 하며, `NotImplementedException`을 운영 실패 흐름의 대체로 쓰지 않는다.
- 기존 `docs/usecase-spec.md`, `docs/favorites-usecase-eventstorming.md`, `docs/road-name-search-usecase-eventstorming.md`, README의 문서 연결을 해치지 않는다.
